...
Game::Game() : inventory_("Inventory") {
  ...
  */
  ...
  get_handler = new ActionHandler(
      Action::kGet, this, [](Game* game, const Item* item) {
  ...
  drop_handler = new ActionHandler(
      Action::kDrop, this, [](Game* game, const Item* item) {
        if (!game->inventory_.HasItem(item)) {
  ...
  items_ = {
    ...
    Item("locked door", 20, {}),
    Item("door", 1, {}), Item("bell", 2, {}),
    Item("scratches", 6, {}), Item("key cutter", 15, {}),
    Item("hole", 10, {}), Item("transmitter", 7, {}),
    Item("window", 18, {}), Item("mud man", 11, {}),
    Item("wire", 19, {}), Item("inscription", 13, {}),
...
void Game::Run() {
  ...
  // Print instructions
  std::cout << " You have been captured by creatures on an uncharted planet." <<
    std::endl << " Unfortunately the planet happens to be unstable,and has " <<
    "been evacuated." << std::endl << " You therefore have to escape before " <<
    "the planet blows up with you on it." << std::endl << " The computer " <<
    "has a fairly large number of commands,so if one command does not work " <<
    "then try another." << std::endl << " The first three letters of each " <<
    "command and object need be typed in, although, if desired, the full " <<
    "word may be entered." << std::endl;
  ...
  while(running) {
    ...
    for(auto e: {Direction::kNorth, Direction::kSouth, Direction::kEast, 
                 Direction::kWest}) {
      if (current_room->HasExit(e))
        std::cout << ':' << e << ':';
    }

    std::cout << std::endl << std::endl << "Objects:- ";
    for(auto item: current_room->items())
      std::cout << item->name() << ":";
...
Direction Game::MakeDirection(const Command cmd) const {
  switch(cmd.action()) {
    case Action::kMoveNorth: return Direction::kNorth;
    case Action::kMoveSouth: return Direction::kSouth;
    case Action::kMoveEast: return Direction::kEast;
    case Action::kMoveWest: return Direction::kWest;
